...
class Order(Document):
        ...
        if self.workflow_state == "Cancelled":
            ...
            for ts in timesheets:
                if ts.docstatus in (0, 1):
                    ...
                    frappe.msgprint(
                        f"All linked Timesheets for Order {self.name} have been cancelled."
                    )

    def on_submit(self):
        item_variant = create_line_items(self)
        if self.bom_or_cad == "Duplicate BOM" or (
            ...
            if self.mod_reason == "Change in Metal Touch":
                ...
                frappe.msgprint(
                    _("New BOM Created: {0}".format(get_link_to_form("BOM", new_bom)))
                )
                self.reload()
            else:
    ...
    def on_cancel(self):
        if frappe.db.get_list("Timesheet", filters={"order": self.name}, fields="name"):
            for timesheet in frappe.db.get_list(
                "Timesheet", filters={"order", self.name}, fields="name"
            ):
...
def bom_creation(self):
    ...
    if self.design_type != "Mod - Old Stylebio & Tag No":
        existing_boms = frappe.get_all(
            "BOM", filters={"item": self.item, "docstatus": 1}, fields=["name"]
        )
        if existing_boms:
            frappe.throw(
                f"BOM already exists for item {self.item}. Multiple BOMs are allowed only for 'Mod' design type."
            )
    ...
    bom.append("items", {"item_code": self.item, "qty": self.qty})
...
def calculate_metal_weights(self):
    self.total_metal_weight = sum(
        i.finish_product_weight or 0 for i in self.metal_detail
    )
...
def create_timesheet(self):
    ...
    for row in self.designer_assignment:
        ...
        existing_ts = frappe.get_all(
            "Timesheet", filters={"employee": row.designer, "order": self.name}, limit=1
        )
...
def timesheet_validation(self):
    if self.cad_order_form and self.workflow_state in [
        "Update Item",
        "Design Rework in Progress",
    ]:
        ...
        if self.workflow_state == "Update Item":
            if required_approval:
                ...
                frappe.db.commit()
                frappe.msgprint(
                    f"All relevant Timesheets are now auto-approved for Order {self.name}. Proceeding."
                )
            else:
                not_approved = [
                    ts["name"]
            ...
            for ts in timesheets:
                if ts["workflow_state"] == "Approved" and ts["docstatus"] != 2:
                    frappe.db.set_value(
                        "Timesheet",
                        ts["name"],
                        {"workflow_state": "Design Rework in Progress", "docstatus": 0},
                    )
            frappe.db.commit()
            frappe.msgprint(
                f"Relevant Timesheets for Order {self.name} updated to 'Design Rework in Progress'."
            )


def close_last_timelog(ts_name):
    ...
    if last_log and last_log[1]:
        now = now_datetime()
        hours = (now - last_log[1]).total_seconds() / 3600

        frappe.db.set_value(
            "Timesheet Detail",
            last_log[0],
            {"to_time": now, "hours": hours, "completed": 1},
        )
...
def validate_timesheet(self):
    if self.customer_order_form:
        return

    def close_last_log(ts_doc):
        if ts_doc.time_logs:
            last = ts_doc.time_logs[-1]
            if not last.to_time:
                last.to_time = now_datetime()
                last.hours = (last.to_time - last.from_time).total_seconds() / 3600

    def get_timesheet(designer):
        ts = frappe.get_all(
            "Timesheet",
            filters={"employee": designer, "order": self.name},
            fields=["name"],
            limit=1,
        )
        return frappe.get_doc("Timesheet", ts[0].name) if ts else None

    if self.workflow_state == "Designing":
        if len(self.designer_assignment) > 1:
            for row in self.designer_assignment[:-1]:
                ts_doc = get_timesheet(row.designer)
                if ts_doc and ts_doc.docstatus == 0:
                    close_last_log(ts_doc)
                    ts_doc.save()
                    ts_doc.submit()

        designer = self.designer_assignment[-1].designer
        ts_doc = get_timesheet(designer)

        if not ts_doc:
            ts_doc = frappe.new_doc("Timesheet")
            ts_doc.employee = designer
            ts_doc.order = self.name

        close_last_log(ts_doc)

        log = ts_doc.append("time_logs", {})
        log.activity_type = "CAD Designing"
        log.from_time = now_datetime()
        ...
        log = ts_doc.append("time_logs", {})
        log.activity_type = "QC Activity"
        log.from_time = now_datetime()
        log.custom_cad_order_id = self.name
        ...
        if (
            not ts_doc.time_logs
            or ts_doc.time_logs[-1].activity_type != "CAD Designing - On-Hold"
        ):
            log = ts_doc.append("time_logs", {})
            log.activity_type = "CAD Designing - On-Hold"
            log.from_time = now_datetime()
            log.custom_cad_order_id = self.name
    ...
    elif self.workflow_state == "Update Item" and self.bom_or_cad == "CAD":
        designer = self.designer_assignment[-1].designer
        ts_doc = get_timesheet(designer)
        ...
        log = ts_doc.append("time_logs", {})
        log.activity_type = "Update Item"
        log.from_time = now_datetime()
        log.custom_cad_order_id = self.name

        ts_doc.save()
        frappe.msgprint(
            "Timesheets created for Update Item for each designer assignment"
        )

    elif self.workflow_state == "Update BOM" and self.bom_or_cad == "CAD":
        designer = self.designer_assignment[-1].designer
        ts_doc = get_timesheet(designer)
        ...
        for row in self.designer_assignment:
            if row.designer:
                timesheets = frappe.db.get_list(
                    "Timesheet",
                    filters={"order": self.name, "employee": row.designer},
                    fields=["name"],
                )
                for ts in timesheets:
                    frappe.db.set_value(
...
def close_last_bom_timelog(ts_name, mark_completed=False):
    last_log = frappe.db.get_value(
        "Timesheet Detail",
        {"parent": ts_name},
        ["name", "from_time"],
    ...
    if last_log and last_log[1]:
        now = now_datetime()
        data = {"to_time": now, "hours": (now - last_log[1]).total_seconds() / 3600}
        if mark_completed:
            data["completed"] = 1

        frappe.db.set_value("Timesheet Detail", last_log[0], data)


def create_bom_timesheet(self):
    if self.customer_order_form:
        return

    if self.bom_or_cad not in ["CAD", "Check"]:
        return

    now = now_datetime()
    assignments = self.bom_assignment or []
        ...
        if len(assignments) > 1:
            for row in assignments[:-1]:
                ts = frappe.db.get_value(
                    "Timesheet",
                    {"employee": row.designer, "order": self.name, "docstatus": 0},
                    "name",
                )
                if ts:
    ...
    def get_active_ts(designer):
        ts = frappe.db.get_value(
            "Timesheet", {"employee": designer, "order": self.name}, "name"
        )
        if not ts:
            frappe.throw("Timesheets is not created for each designer assignment")
        return ts
    ...
    if self.workflow_state == "Creating BOM":
        ...
        designer = assignments[-1].designer
        ts = frappe.db.get_value(
            "Timesheet", {"employee": designer, "order": self.name}, "name"
        )
        ...
        last_activity = frappe.db.get_value(
            "Timesheet Detail", {"parent": ts}, "activity_type", order_by="idx desc"
        )
        ...
        if last_activity != "Create BOM - On Hold":
            frappe.get_doc(
                {
                    "doctype": "Timesheet Detail",
                    "parent": ts,
                    "parenttype": "Timesheet",
                    "parentfield": "time_logs",
                    "activity_type": "Create BOM - On Hold",
                    "from_time": now,
        ...
        if last_activity != "BOM QC - On Hold":
            frappe.get_doc(
                {
                    "doctype": "Timesheet Detail",
                    "parent": ts,
        ...
        if last_activity != "Updating BOM - On Hold":
            frappe.get_doc(
                {
                    "doctype": "Timesheet Detail",
                    "parent": ts,
                    "parenttype": "Timesheet",
                    "parentfield": "time_logs",
                    "activity_type": "Updating BOM - On Hold",
                    "from_time": now,
                    "custom_cad_order_id": self.name,
                }
            ).insert(ignore_permissions=True)
...
def create_line_items(self):
    sketch_order_form_id = frappe.db.get_value(
        "Item", self.design_id, "custom_sketch_order_id"
    )
    custom_sketch_order_form_id = frappe.db.get_value(
        "Item", self.design_id, "custom_sketch_order_form_id"
    )
    ...
    if self.cad_order_form:
        order_type = frappe.db.get_value(
            "Order Form", self.cad_order_form, "order_type"
        )
        if order_type == "Purchase":
            purchase_type_for_design = frappe.db.get_value(
                "Order Form", self.cad_order_form, "purchase_type"
            )
            supplier_for_design = frappe.db.get_value(
                "Order Form", self.cad_order_form, "supplier"
            )

    # block item creation
    if (
        self.design_type in ["Sketch Design", "New Design"]
        and self.item_remark == "Copy Paste Item"
    ):
        frappe.msgprint(
            _(
                "Item creation skipped because Design Type is {0} and Item Remark is Copy Paste Item"
    ...
    if self.item_type == "Template and Variant":
        ...
        if supplier_for_design:
            frappe.db.set_value(
                "Item", item_variant, "design_supplier", supplier_for_design
            )
        ...
        frappe.msgprint(
            _("New Item Created: {0}".format(get_link_to_form("Item", item_variant)))
        )
        ...
        if template:
            ...
            for variant_code in actual_variants:
                ...
                if variant:
                    frappe.throw(
                        f"Already available <a href='/app/item/{variant.name}'>{variant.name}</a>"
                    )
        ...
        new_item_created = True
        frappe.msgprint(
            _("New Item Created: {0}".format(get_link_to_form("Item", item_variant[0])))
        )
        ...
        if not self.is_finding_order:
            ...
            if self.bom_or_cad in ["New BOM", "CAD"] and self.is_repairing == 1:
                ...
                update_variant_attributes(self)
                frappe.db.set_value(
                    "Item", self.design_id, "custom_cad_order_id", self.name
                )
                frappe.db.set_value(
                    "Item",
                    self.design_id,
            ...
            frappe.db.set_value(
                "Item", self.design_id, "custom_sketch_order_id", sketch_order_form_id
            )
            frappe.db.set_value(
                "Item",
                item_variant,
    ...
    frappe.db.set_value(
        ...
        self.name,
        "item_remark",
        "New Item" if new_item_created else "Copy Paste Item",
    )
...
def create_only_variant_from_order(self, source_name, target_doc=None):
    def post_process(source, target):
        db_data = frappe.db.get_list(
            "Item",
            filters={"name": self.design_id},
            fields=["variant_of", "item_group"],
        ...
        if db_data["variant_of"]:
            db_data1 = frappe.db.get_list(
                "Item",
                filters={"variant_of": db_data["variant_of"]},
                fields=["name"],
                order_by="creation desc",
            )[0]
...
def make_quotation(source_name, target_doc=None):
    ...
    def set_missing_values(source, target):
        ...
        service_types = frappe.db.get_values(
            "Service Type 2", {"parent": source.name}, "service_type1"
        )
...
def update_variant_attributes(self):
    variant_attributes = frappe.db.sql(
        f"""select name,attribute,attribute_value from `tabItem Variant Attribute` where parent = '{self.design_id}'""",
        as_dict=1,
    )
...
def check_finding_code(self):
    ...
    if variant:
        ...
        variant_item_group = frappe.db.get_value(
            "Variant Item Group",
            {"parent": self.company, "item_variant": "F"},
            "item_group",
        )
...
def calculate_item_wt_details(doc, bom=None, item=None):
    ...
    cad_to_rpt, rpt_to_wax = frappe.get_value(
        "Jewellery Settings", None, ["cad_to_rpt", "rpt_to_wax"]
    )

    doc["cam_weight"] = flt(doc["cad_weight"]) / flt(cad_to_rpt or 1)
    doc["wax_weight"] = flt(doc["cam_weight"]) / flt(rpt_to_wax or 1)
    ratio_dict = {
        "22KT": "18",
        "18KT": "16",
        "14KT": "14.23",
...
def make_atribute_list(source_name):
    order_details = frappe.get_value("Order", source_name, "*", as_dict=True)

    all_variant_attribute = frappe.db.sql(
        """
		SELECT item_attribute
		FROM `tabAttribute Value Item Attribute Detail`
		WHERE parent = %s AND in_item_variant = 1
		""",
        (order_details.subcategory,),
        as_list=True,
    )
    ...
    for i in all_variant_attribute:
        attr = i[0]
        fieldname = attr.replace(" ", "_").replace("/", "").lower()

        if fieldname == "rhodium":
            fieldname = "rhodium_"

        final_list[attr] = order_details.get(fieldname)
