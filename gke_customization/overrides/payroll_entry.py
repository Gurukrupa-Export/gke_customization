...
class CustomPayrollEntry(PayrollEntry):
    ...
    def get_statistical_components(self) -> set[str]:
        ...
        return set(
            frappe.get_all(
                "Salary Component", filters={"statistical_component": 1}, pluck="name"
            )
        )
    ...
    @frappe.whitelist()
    def get_pending_release_entries(self) -> list[str]:
        """Return draft Journal Entries that block additional salary releases.

        The client uses this to explain why the Release Withheld Salary button is
        hidden until the draft entries are submitted or deleted.
        """
        Release = frappe.qb.DocType("Salary Withholding Release")
        JournalEntry = frappe.qb.DocType("Journal Entry")
    ...
    def get_salary_slip_details(
        ...
        if for_withheld_salaries:
            ...
            if pending_release := self.get_withholding_cycles_pending_release():
                query = query.where(
                    SalarySlip.salary_withholding_cycle.notin(pending_release)
                )
        else:
            query = query.where(SalarySlip.status != "Withheld")
    ...
    def make_bank_entry(
        ...
        for payable in employee_payables.values():
            payable["net_payable"] = (
                flt(payable["earnings"])
                - flt(payable["deductions"])
                - flt(payable["loan_repayment"])
            )

        if for_withheld_salaries and release_amounts:
            release_amounts = self.validate_release_amounts(
                release_amounts, employee_payables
            )
    ...
    def validate_release_amounts(
        ...
        for employee, amount in release_amounts.items():
            ...
            if amount <= 0:
                frappe.throw(
                    _(
                        "Release amount for employee {0} must be greater than zero"
            ...
            if flt(amount, precision) > flt(payable["net_payable"], precision):
                frappe.throw(
                    _(
                        "Release amount for employee {0} ({1}) cannot exceed the pending amount {2}"
                    ).format(frappe.bold(employee), amount, payable["net_payable"]),
                    title=_("Invalid Release Amount"),
                )
        ...
        if employee_wise_accounting_enabled:
            self.employee_based_payroll_payable_entries = {
                employee: {
                    "earnings": amount,
                    "deductions": 0,
                    "salary_structure": employee_payables.get(employee, {}).get(
                        "salary_structure"
                    ),
                }
                for employee, amount in release_amounts.items()
            ...
            frappe.get_doc(
                {
                    ...
                    "parent": frappe.db.get_value(
                        "Salary Withholding Cycle", cycle, "parent"
                    ),
