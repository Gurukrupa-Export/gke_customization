...
def execute(filters=None):
	...
	group_wise_columns = frappe._dict(
		{
			"invoice": [
				"parent",           # NEW: dedicated Sales Invoice No column (repeats on every row)
				"invoice_or_item",
				"customer",
				"customer_group",
				"customer_name",
...
def get_data_when_grouped_by_invoice(columns, gross_profit_data, filters, group_wise_columns, data):
	...
	# NOTE: The original code used to overwrite columns[0] and delete item_code/item_name
	# columns because everything was crammed into invoice_or_item. Now that we have:
	#   - "parent"   → a dedicated Sales Invoice No column (always repeating)
	#   - "batch_no" → an explicit Batch No column
	#   - "item_code" / "item_name" kept as separate columns
	# ...we no longer need those hacks. All columns are correctly defined in get_columns().
...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict(
		{
			# NEW: dedicated Sales Invoice No column — always shows SI name, repeating per item row
			"parent": {
...
def get_column_names():
	return frappe._dict(
		{
			"parent": "parent_invoice",       # NEW: maps "parent" key to parent_invoice fieldname
			...
			"warehouse": "warehouse",
			"batch_no": "batch_no",           # NEW: Batch No mapping
			"qty": "qty",
...
class GrossProfitGenerator:
	...
	def group_items_by_invoice(self):
		...
		for row in self.si_list:
			# No header/summary row — item rows already carry all SI details via parent_invoice
	...
	def get_invoice_row(self, row):
		# Header row for each Sales Invoice group.
		# parent_invoice is now set to the actual SI name (was "") so the dedicated
		# "Sales Invoice" column repeats the SI No on every row including the header.
		return frappe._dict(
			{
				"parent_invoice": row.parent,   # CHANGED: was "" — now shows SI No on header row
				"indent": 0.0,
		...
		return frappe._dict(
			{
				...
				"item_group": "",
				"brand": "",
				"batch_no": row.get("batch_no"),     # NEW: inherit batch_no from parent SI item row
