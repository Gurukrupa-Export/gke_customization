frappe.query_reports["Main Slip Details"] = {
    "filters": [
        {
            fieldname: "from_date",
            label: __("From Date"),
    ...
    formatter: function (value, row, column, data, default_formatter) {
        ...
        if (column.fieldname === "emp_code" && data && data.employee) {
            value = `<span title="${frappe.utils.escape_html(data.employee)}">${value}</span>`;
        }
        return value;
    },
    onload: function (report) {
        const department_filter = report.get_filter("department");
        if (!department_filter) return;

        const can_change_department = frappe.user.has_role(["System Manager", "Administrator"]);
        ...
        frappe.call({
            ...
            callback: function (r) {
                ...
                if (!can_change_department) {
                    department_filter.df.read_only = 1;
...
function show_main_slip_details(main_slip_no) {
    frappe.call({
        method: "gke_customization.gke_catalog.report.main_slip_details.main_slip_details.get_main_slip_detail_popup",
