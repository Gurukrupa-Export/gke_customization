	...
	columns.extend(
		[
			...
			{"label": _("Balance Qty"), "fieldname": "balance_qty", "fieldtype": "Float", "width": 150},
			{
				...
				"fieldname": "custom_certificate_id",
				"fieldtype": "Data",
				"width": 150,
			},
		]
...
def get_data(filters):
	...
	set_batchwise_certificate_id(data)
...
def set_batchwise_certificate_id(data):
	...
	certificate_id_map = frappe._dict(
		frappe.get_all(
			"Purchase Receipt Item",
			filters={"batch_no": ("in", list(batch_nos))},
			fields=["batch_no", "custom_certificate_id"],
			as_list=True,
		)
	)

	for d in data:
		d["custom_certificate_id"] = certificate_id_map.get(d.batch_no)
