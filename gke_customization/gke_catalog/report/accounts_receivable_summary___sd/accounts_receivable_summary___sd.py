...
class AccountsReceivableSummary(ReceivablePayableReport):
	...
	def get_data(self, args):
		self.data = []
		inner_report = ReceivablePayableReport(self.filters)
		self.receivables = inner_report.run(args)[1]
		self.summary_type = self.filters.get("summary_type") or "Customer Summary"

		if self.summary_type == "Monthly Summary":
			self.setup_monthly_fieldnames()
		else:
			self.date_fieldnames = inner_report.date_fieldnames
	...
	def setup_date_columns(self):
		date_format = "MMM yyyy" if self.summary_type == "Monthly Summary" else None
		for date, fieldname in self.date_fieldnames.items():
			self.add_column(label=formatdate(date, date_format), fieldname=fieldname)
