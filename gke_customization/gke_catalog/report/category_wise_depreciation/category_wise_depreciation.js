...
frappe.query_reports["Category wise Depreciation"] = {
    formatter: function (value, row, column, data, default_formatter) {
        value = default_formatter(value, row, column, data);

        if (column.fieldname === "depreciation") {
            ...
            // Construct JSON filter format: ["in", ["val1", "val2"]]
            ...
            let category = data && data.category ? data.category.replace(/(<([^>]+)>)/gi, "") : "";
            let category_param = category ? `&asset_category=${encodeURIComponent(category)}` : "";
