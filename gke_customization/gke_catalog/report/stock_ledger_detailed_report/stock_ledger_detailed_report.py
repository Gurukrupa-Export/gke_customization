...
# Branch Wise Stock Ledger - Filters by Branch via Stock Entry join and shows Branch column
...
def execute(filters=None):
    ...
    if opening_row:
        ...
        actual_qty = opening_row.get("qty_after_transaction") or 0
        stock_value = opening_row.get("stock_value") or 0
...
def get_stock_ledger_entries(filters, items):
    ...
    query = (
        frappe.qb.from_(sle)
        .left_join(se).on(sle.voucher_no == se.name)
        .select(
            sle.item_code, sle.posting_datetime.as_("date"), sle.warehouse,
            sle.posting_date, sle.posting_time, sle.actual_qty,
            sle.incoming_rate, sle.valuation_rate, sle.company,
            se.branch.as_("branch"),  # <- branch from Stock Entry
            sle.voucher_type, sle.qty_after_transaction, sle.stock_value_difference,
            sle.serial_and_batch_bundle, sle.voucher_no, sle.stock_value,
...
def get_warehouse_condition(warehouse):
    ...
    if warehouse_details:
        return f""" exists (
            ...
            where wh.lft >= {warehouse_details['lft']}
            and wh.rgt <= {warehouse_details['rgt']}
...
def get_columns(filters):
    ...
    columns.extend([
        ...
        {"label": _("Avg Rate (Balance Stock)"), "fieldname": "valuation_rate", "fieldtype": filters.valuation_field_type, "width": 180, "options": "Company:company:default_currency" if filters.valuation_field_type == "Currency" else None, "convertible": "rate"},
        {"label": _("Valuation Rate"), "fieldname": "in_out_rate", "fieldtype": filters.valuation_field_type, "width": 140, "options": "Company:company:default_currency" if filters.valuation_field_type == "Currency" else None, "convertible": "rate"},
        ...
        {"label": _("Serial and Batch Bundle"), "fieldname": "serial_and_batch_bundle", "fieldtype": "Link", "options": "Serial and Batch Bundle", "width": 100},
...
def get_item_group_condition(item_group, item_table=None):
    ...
    if item_group_details:
        if item_table:
            ...
            return f"""item.item_group in (
                select ig.name from `tabItem Group` ig
                where ig.lft >= {item_group_details.lft}
