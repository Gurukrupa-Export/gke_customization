...
def get_columns():
    columns = [
        ...
        {"fieldname": "total_qty", "label": _( "Total Qty"), "fieldtype": "Data", "width": 120},
...
ORDER BY so.name DESC, so.transaction_date DESC
    ...
    for row in data:
        ...
        row["form_name"] = f'<a href="https://gkexport.frappe.cloud/app/sales-order/{encoded_form_name}" target="_blank">{row["form_name"]}</a>'
...
def calculate_status_duration(modified_dt, status):
    ...
    exceeded = hrs_count > 5 and status not in ["<span style='color:inherit; font-weight:bold;'>Cancelled</span>",f'<span style="color:green;font-weight:bold;">Completed</span>',f'<span style="color:green;font-weight:bold;">Closed</span>']
...
def get_message():
	...
	return """<span class="indicator" style="font-weight: bold; font-size: 15px;">
        ...
        The standard deadline for this process is 5 hrs.
        </span>
        &nbsp;&nbsp;&nbsp;
        <span class="indicator blue" style="font-size: 15px;margin-left: 176px;">
        Time difference = Created date-time - Status date-time.
        </span>
        <br>
        &nbsp;
        <span class="indicator red" style="font-size: 15px; margin-left: 66px;">
        Delivery Date turns red if delivery date is updated.
        </span>
        <span class="indicator blue" style="font-size: 15px; margin-left: 170px;">
        Status Duration = Status date-time - Current date-time.
        </span>
        <br>
        <span class="indicator yellow" style="font-size: 15px; margin-left: 73px;">
        Number of Items: Quantity of each item in Order
        </span>
        <span class="indicator yellow" style="font-size: 15px; margin-left: 183px;">
        Total Qty = Quantity of Items in Sales Order.
        </span>
"""
