...
def execute(filters=None):
    ...
    # Fetch loans
    loan_query = f"""
        SELECT name, lender, loan_amount, loan_date, interest_rate
        FROM `tabUnsecured Loan`
        WHERE 1=1 {loan_conditions}
    """
    ...
    for loan in loans:
        ...
        repayments = frappe.db.sql("""
            SELECT 
                ...
                uls.payment_entry_created   -- ✅ from schedule table
        ...
        if not found_initial_pe:
            ...
            tx_rows.append([
                loan.name, balance, initial_date, next_tx_date,
                days, loan.interest_rate, interest_amount,
                "Receive", flt(loan.loan_amount),
                0  # synthetic row, no PE tick
            ])
            start_idx = 0
        else:
            start_idx = 0
        ...
        for i in range(start_idx, n):
            ...
            tx_rows.append([
                ...
                r.payment_type,
                flt(r.total_payment),
                r.payment_entry_created or 0  # ✅ tick based on field from schedule
            ])
...
def get_columns():
    return [
        ...
        {"label": "PE Created", "fieldname": "pe_created", "fieldtype": "Check", "width": 100}  # check/tick
    ]
