...
class SketchOrder(Document):
	...
	def make_items(self):
		...
		for row in self.final_sketch_approval_cmo:
			...
			frappe.msgprint(
				_(
					"New Item Created: {0}".format(
						get_link_to_form("Item", item_variant)
					)
				)
			)
...
def create_item_from_sketch_order(self, item_template, source_name, target_doc=None):
	def post_process(source, target):
		...
		target.age_group = self.age_group
		target.alphabetnumber = self.alphabetnumber
		target.animalbirds = self.animalbirds
		target.collection = self.collection_1
		target.design_style = self.design_style
		target.gender = self.gender
		target.lines_rows = self.lines_rows
		target.language = self.language
		target.occasion = self.occasion
		target.religious = self.religious
		target.shapes = self.shapes
		target.zodiac = self.zodiac
		target.rhodium = self.rhodium
		...
		for i in frappe.get_all(
			...
			try:
				attribute_value = frappe.db.get_value(
					"Sketch Order", self.name, attribute_with
				)
			except:
				attribute_value = ""
