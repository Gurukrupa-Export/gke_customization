...
from gke_customization.gke_catalog.api.encryption_response import SecureJSON
import pyotp
import hmac
import hashlib
import base64
...
def get_is_filter(search, values, wishlist_case, sub_where, customer_join, where_clause):
    ...
    start = int(values.get("offset", 0))
    page_size = int(values.get("page_size", 48))
    end = start + page_size
    
    enc_data = (db_data[start:end], len(db_data))
...
def catalogue_data22(selectedSubcategory=None, itemCategory=None, itemCode=None, metalType=None, company=None, customer=None, page=1, page_size=48, is_filter=None, search=None):
    ...
    if is_filter == 1:
        ...
        encrypted = get_is_filter(search, values, wishlist_case, sub_where, customer_join, where_clause)
