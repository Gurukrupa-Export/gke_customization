...
class ReviseSupplierServicesPrice(Document):
	def before_save(self):
		if self.supplier and self.purchase_type and self.metal_type and self.type_of_subcontracting:
			...
			if len(self.revise_making_charge_price_item_subcategory) != len(doc.subcategory):
				if self.revise_making_charge_price_item_subcategory==[]:
					...
					for j in sorted_data:
						...
						rate_details.revise_rate_per_gm = j['rate_per_gm']
						
						rate_details.rate_per_diamond = j['rate_per_diamond']
						rate_details.revise_rate_per_diamond = j['rate_per_diamond']
						
						rate_details.rate_per_pc = j['rate_per_pc']
						rate_details.revise_rate_per_pc = j['rate_per_pc']
						
						rate_details.wastage = j['wastage']
						rate_details.revise_wastage = j['wastage']
						
						rate_details.name = j['name']
				else:
					old_supplier_price_list = [d.name for d in self.revise_making_charge_price_item_subcategory]
					sorted_data = []
	...
	def on_submit(self):
		...
		if price_list_id:
			...
			for i in self.revise_making_charge_price_item_subcategory:
				...
				row.rate_per_gm = i.revise_rate_per_gm
				row.rate_per_pc = i.revise_rate_per_pc
				row.rate_per_diamond = i.rate_per_diamond
				row.wastage = i.revise_wastage
