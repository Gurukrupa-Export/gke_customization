...
class UpdateMakingChargePrice(Document):
    def before_save(self):
        ...
        if self.making_charge_type == 'Regular':
            ...
            if len(self.update_making_charge_price_item_subcategory) != len(frappe.get_doc("Making Charge Price",name).subcategory):
                if self.update_making_charge_price_item_subcategory==[]:
                    ...
                    sorted_data = []
                    for j in frappe.db.get_list("Making Charge Price Item Subcategory",filters={'parent':name},pluck="name"):
    ...
    def on_submit(self):
        ...
        if self.making_charge_type == 'Regular':
            ...
            frappe.db.set_value('Making Charge Price',name,'effective_from',self.date)
