...
class UpdateGemstonePriceList(Document):
    def before_save(self):
        ...
        if self.price_list_type == 'Weight (in cts)':
            if len(self.update_gemstone_price_list_details) != len(frappe.db.get_list("Gemstone Price List",filters=filters,fields=["from_weight","to_weight"])):
                if self.update_gemstone_price_list_details==[]:
                    self.set("update_gemstone_price_list_details", [])
...
def get_value(doc):
    ...
    if json_doc['price_list_type'] == 'Weight (in cts)':
        for i in json_doc['update_gemstone_price_list_details']:
        ...
        doc1 = frappe.get_doc('Update Gemstone Price List',json_doc['name'])
        for j in doc1.update_gemstone_price_list_details:
            frappe.db.set_value('Update Gemstone Price List Details',j.name,'idx',sorted_ranges.index(j.weight))
