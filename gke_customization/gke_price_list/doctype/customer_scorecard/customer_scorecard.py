...
from gke_customization.gke_price_list.doctype.customer_scorecard_period.customer_scorecard_period import (
	make_customer_scorecard,
)

class CustomerScorecard(Document):
	...
	if TYPE_CHECKING:
		...
		from gke_customization.gke_price_list.doctype.customer_scorecard_scoring_standing.customer_scorecard_scoring_standing import (
			CustomerScorecardScoringStanding,
		)
		...
		standings: DF.Table[CustomerScorecardScoringStanding]
		# status: DF.Data | None
		customer: DF.Link | None
		customer_score: DF.Data | None
		# warn_pos: DF.Check
		# warn_rfqs: DF.Check
	...
	def calculate_total_score(self):
		scorecards = frappe.db.sql(
			...
			SELECT
				scp.name
			FROM
				`tabCustomer Scorecard Period` scp
			WHERE
				scp.scorecard = %(sc)s
				AND scp.docstatus = 1
			ORDER BY
		...
		for scp in scorecards:
			my_sc = frappe.get_doc("Customer Scorecard Period", scp.name)
		...
		if total_max_score > 0:
			self.customer_score = round(100.0 * (total_score / total_max_score), 1)
		else:
			self.customer_score = 100
...
def get_timeline_data(doctype, name):
	...
	scorecards = frappe.db.sql(
		...
		SELECT
			sc.name
		FROM
			`tabCustomer Scorecard Period` sc
		WHERE
			sc.scorecard = %(scs)s
			AND sc.docstatus = 1""",
		{"scs": scs.name},
	...
	for sc in scorecards:
		start_date, end_date, total_score = frappe.db.get_value(
			"Customer Scorecard Period", sc.name, ["start_date", "end_date", "total_score"]
		)
...
def refresh_scorecards():
	...
	for sc in scorecards:
		...
		if make_all_scorecards(sc.name) > 0:
			...
			frappe.get_doc("Customer Scorecard", sc.name).save()


@frappe.whitelist()
def make_all_scorecards(docname):
	sc = frappe.get_doc("Customer Scorecard", docname)
	customer = frappe.get_doc("Customer", sc.customer)

	start_date = getdate(customer.creation)
	...
	while (start_date < todays) and (end_date <= todays):
		...
		if len(scorecards) == 0:
			period_card = make_customer_scorecard(docname, None)
	...
	if scp_count > 0:
		frappe.msgprint(
			_("Created {0} scorecards for {1} between:").format(scp_count, sc.customer)
			+ " "
...
def make_default_records():
	...
	for d in install_variable_docs:
		try:
			d["doctype"] = "Customer Scorecard Variable"
			frappe.get_doc(d).insert()
		except frappe.NameError:
			pass
	for d in install_standing_docs:
		try:
			d["doctype"] = "Customer Scorecard Standing"
			frappe.get_doc(d).insert()
		except frappe.NameError:
			pass
