...
# Fallback when invoice_sales_type isn't set — matches an E Invoice Item's
# Sales Type Multiselect vocabulary (Outright / Outwork / Branch Sales /
# Hybrid / Certification / Repairing), not doc.sales_type (a PRF-specific
# return-category label like "Finished Goods Return" that never appears
# there).
TEMP_SALES_TYPE = "Outright"
...
def validate_e_invoice_items(doc):
	...
	# Current Sales Type
	sales_type = doc.invoice_sales_type or TEMP_SALES_TYPE
	...
	# --- 4b. Hybrid: fold metal/finding "making" buckets into "labour" ---
	# Mirrors Sales Order's single-document Hybrid logic: making charge is
	# always job-work (Outwork) value for Hybrid, so it belongs in the
	# labour/Subcontracting bucket, not listed separately as its own
	# "XX Making Charges" line (which is the Outright-taxed bucket).
	if doc.invoice_sales_type == "Hybrid":
		making_item_types = {
			c["item_type"] for c in e_invoice_config_items
			if c.get("is_for_making") or c.get("is_for_finding_making")
		}
		for key in list(aggregated_data.keys()):
			item_code, uom = key
			if item_code not in making_item_types:
				continue
			entry = aggregated_data.pop(key)
			if not entry.get("amount"):
				continue
			labour_item = find_e_item_by_criteria(is_for_labour=1)
			if not labour_item:
				continue
			target_key = (labour_item["item_type"], labour_item["uom"])
			if target_key not in aggregated_data:
				aggregated_data[target_key] = {
			...
			target = aggregated_data[target_key]
			target["qty"] += entry["qty"]
			target["amount"] += entry["amount"]
			target["tax_amount"] = target["amount"] * (target["tax_rate"] / 100.0)
