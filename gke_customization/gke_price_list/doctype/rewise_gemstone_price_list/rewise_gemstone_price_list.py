...
class RewiseGemstonePriceList(Document):
    def before_save(self):
        ...
        if self.price_list_type == 'Weight (in cts)':
            if len(self.rewise_gemstone_price_list_details) != len(frappe.db.get_list("Gemstone Price List",filters=filters,fields=["from_weight","to_weight"])):
                if self.rewise_gemstone_price_list_details==[]:
                    self.set("rewise_gemstone_price_list_details", [])
                    ...
                    old_gemstone_price_list = [d.gemstone_price_list for d in self.rewise_gemstone_price_list_details]
    ...
    def on_submit(self):
        if self.price_list_type == 'Weight (in cts)':
            for i in self.rewise_gemstone_price_list_details:
                # if i.difference!=0:
...
def for_weight_in_cts(self,i):
    ...
    rate_details = self.append("rewise_gemstone_price_list_details", {})
...
def get_value(doc):
    ...
    if json_doc['price_list_type'] == 'Weight (in cts)':
        for i in json_doc['rewise_gemstone_price_list_details']:
        ...
        doc1 = frappe.get_doc('Rewise Gemstone Price List',json_doc['name'])
        for j in doc1.rewise_gemstone_price_list_details:
            frappe.db.set_value('Rewise Gemstone Price List Details',j.name,'idx',sorted_ranges.index(j.weight))
