...
class CustomerScorecardCriteria(Document):
...
def get_criteria_list():
	criteria = frappe.db.sql(
		...
		SELECT
			scs.name
		FROM
			`tabCustomer Scorecard Criteria` scs""",
		{},
		as_dict=1,
	)
...
def get_variables(criteria_name):
	criteria = frappe.get_doc("Customer Scorecard Criteria", criteria_name)
		...
		for _dummy2 in range(0, len(match.groups())):
			try:
				var = frappe.db.sql(
					...
					FROM
						`tabCustomer Scorecard Variable` scv
					WHERE
						param_name=%(param)s""",
					{"param": match.group(1)},
					as_dict=1,
