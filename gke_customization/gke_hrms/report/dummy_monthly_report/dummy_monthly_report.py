...
# 					TIMESTAMP(ADDDATE(Date(Attendance.in_time), 1), TIME(Attendance.out_time)),
# 					Attendance.out_time
# 				),
# 				Attendance.in_time
# 			).as_('spent_hours'),
...
# 				# Deduction for time spent AFTER shift end.
# 				# Check if shift is overnight (Start Time > End Time).
# 				# If yes, Shift End Timestamp = Date(In Time) + 1 day + End Time.
# 				# If no, Shift End Timestamp = Date(In Time) + End Time.
# 				- IF(
# 						Attendance.out_time > IF(
...
# 						),
# 						TIME_TO_SEC(
# 							TIMEDIFF(Attendance.out_time, IF(
# 							ShiftType.start_time > ShiftType.end_time, 
# 							TIMESTAMP(ADDDATE(Date(Attendance.in_time), 1), ShiftType.end_time),
# 							TIMESTAMP(Date(Attendance.in_time), ShiftType.end_time)
# 							)
...
# 	# frappe.throw(f"{data}")
# 	frappe.log_error(message=f"Data fetched for attendance report: \n\n {data}", title=f'Attendance Report {frappe.utils.now()}')
...
def get_data(filters=None):
	...
	pol_subquery = (
		...
		.select(
			...
			PersonalOutLog.date,
			SEC_TO_TIME(
				IfNull(Sum(TIME_TO_SEC(PersonalOutLog.total_hours)), 0)
			).as_("hrs")
		)
		.where(PersonalOutLog.is_cancelled == 0)
		.groupby(PersonalOutLog.employee, PersonalOutLog.date)
