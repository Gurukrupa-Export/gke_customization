...
frappe.query_reports["Standard Shift Report"] = {
	"filters": [
		{
			"label": __("Month"),
			"fieldtype": "Select",
	...
	"formatter": function(value, row, column, data, default_formatter) {
		...
		var time_columns = ["in_time", "adj_out_2","total_adj"];
		if (data && !data.attendance_date) {
			value = $(`<span>${value}</span>`);
			var $value = $(value).css("font-weight", "bold");
			value = $value.wrap("<p></p>").parent().html();
		}
		if (in_list(time_columns,column.id) && data.attendance_date) {
			value = frappe.datetime.str_to_user(value, true)
			if (data.late_entry && in_list(["in_time", "adj_out_2"],column.id)) {
				value = $(`<span>${value}</span>`);
				var $value = $(value).css("color", "red");
				value = $value.wrap("<p></p>").parent().html();
			}			
		}
		if (in_list(["adj_out_2", "total_adj"], column.id) && in_list(["Total Hours", "Net Days"],data.adj_out_2)) {
			value = $(`<span>${value}</span>`);
	...
	onload: (report) => {
		...
		report.page.add_button("Clear Filters", function() {
			window.open("/app/query-report/Standard%20Shift%20Report", "_self")
		}).addClass("btn-info");
