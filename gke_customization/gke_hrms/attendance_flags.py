...
def detect_and_apply(doc) -> str | None:
    ...
    # 3) Flag for HR + auto-create the Monthly In-Out Log card so the error
    #    is visible in the report without anyone creating it manually
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "MIL auto-creation failed")
    return error
...
def flag_recent_attendances(days: int = 3):
    ...
    for name in names:
        ...
        # keep existing Monthly In-Out Log cards in sync with the day's
        # current state (ledger, OT check, hours) — no manual Fetch needed
...
def _refresh_mil_card(attendance_doc):
    """Re-populate the employee-date's Monthly In-Out Log card (if one
    exists) so the ledger / OT check / hours reflect the current state.
    Submitted cards are handled by populate's db_set path. Never creates
    a card for clean days — only the flag/resolution paths do that."""
    try:
        ...
        if mil_name:
            frappe.get_doc("Monthly In-Out Log", mil_name).populate_from_attendance()
    except Exception:
        frappe.log_error(
            frappe.get_traceback(),
            f"MIL refresh failed for {attendance_doc.employee}/{attendance_doc.attendance_date}",
        )
