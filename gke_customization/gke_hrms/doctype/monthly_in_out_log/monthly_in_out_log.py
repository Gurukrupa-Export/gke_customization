...
from frappe.query_builder.functions import Concat, Count, Date, IfNull, Sum
from frappe.utils import add_days, flt, format_time, get_datetime, get_time, getdate
from gurukrupa_customizations.gurukrupa_customizations.doctype.manual_punch.manual_punch import (
    get_checkins,  # pyright: ignore[reportMissingImports]
)
...
class MonthlyInOutLog(Document):
    def validate(self):
        ...
        self.shift_hours = frappe.db.get_value(
            "Shift Type", self.shit_type, "shift_hours"
        )
    ...
    def populate_from_attendance(self):
        ...
        try:
            ...
            res = get_attendance_details_by_date(
                self.company, self.employee, attendance_date
            )
            # frappe.throw(f"{res}")
            ...
            self.spent_hrs = fmt_td_or_value(
                record.get("spent_hrs") or record.get("spent_hours")
            )
...
def fetch_attendance_data(filters):
    ...
    for row in raw_data:
        ...
        if row.get("login_date") is None and any(
            k in row for k in ("total_pay_hrs", "ot_hours", "net_wrk_hrs")
        ):
            totals.append(row)
            continue
...
def get_data(filters):
    ...
    pol_subquery = (
        ...
        .select(
            ...
            PersonalOutLog.date,
            SEC_TO_TIME(IfNull(Sum(TIME_TO_SEC(PersonalOutLog.total_hours)), 0)).as_(
                "hrs"
            ),
        )
    ...
    # OT Log subquery
    ot_subquery = (
        frappe.qb.from_(OTLog).select("*").where(OTLog.is_cancelled == 0)
    ).as_("ot")
    ...
    shift_start_with_grace = ADDTIME(
        shift_start, SEC_TO_TIME(ShiftType.late_entry_grace_period * 60)
    )

    effective_in = IF(
        Attendance.in_time <= shift_start_with_grace, shift_start, Attendance.in_time
    )

    effective_out = IF(Attendance.out_time > shift_end, shift_end, Attendance.out_time)
    ...
    query = (
        ...
        .on(
            (
                ...
                & (ShiftAssignment.shift_type == ShiftType.name)
            )
            | (
                (ShiftAssignment.shift_type.isnull())
                & (Attendance.shift == ShiftType.name)
            )
        )
        .left_join(pol_subquery)
        .on(
            (Attendance.attendance_date == pol_subquery.date)
            & (Attendance.employee == pol_subquery.employee)
        )
        .left_join(ot_subquery)
        .on(
        ...
        .select(
            ...
            Concat(
                TIME_FORMAT(ShiftType.start_time, "%H:%i:%s"),
                " TO ",
                TIME_FORMAT(ShiftType.end_time, "%H:%i:%s"),
            ).as_("shift"),
            ...
            IF(
                ...
                TIMEDIFF(
                    Attendance.in_time,
                    TIMESTAMP(Attendance.attendance_date, ShiftType.start_time),
                ),
                None,
            ).as_("late_hrs"),
            IF(
                Attendance.early_exit,
                TIMEDIFF(ShiftType.end_time, TIME(Attendance.out_time)),
                None,
            ).as_("early_hrs"),
            ################################################
            ...
            SEC_TO_TIME(
                IfNull(
                    IF(
                        Attendance.status == "Absent",
                        0,
                        TIME_TO_SEC(TIMEDIFF(effective_out, effective_in))
                        - IfNull(TIME_TO_SEC(pol_subquery.hrs), 0),
                    ),
                    0,
                )
...
def process_data(data, filters):
    ...
    holidays = []
    wo = []
    emp_det = frappe.db.get_value(
        "Employee",
        employee,
        ["default_shift", "holiday_list", "date_of_joining"],
        as_dict=1,
    )
    ...
    #########################################################
    is_night_shift = get_time(shift_det.get("start_time")) > get_time(
        shift_det.get("end_time")
    )
    ...
    if is_night_shift:
        ...
        checkins = (
            ...
            .select(
                IF(
                    TIME(EmployeeCheckin.time)
                    < shift_start_time,  # shift_start_time = 23:00:00 and time =  22:00:00
    ...
    od = frappe.db.sql(
        """
			...
			GROUP BY DATE(time)
		""",
        {"employee": employee, "from_date": from_date, "to_date": add_days(to_date, 1)},
        pluck="login_date",
    )
    ...
    if hl_name := emp_det.get("holiday_list"):
        ...
        holidays = frappe.db.sql(
			...
			""",
            {"hl_name": hl_name, "from_date": from_date, "to_date": to_date},
            as_dict=1,
        )
    ...
    for row in data:
        ...
        if grace_period != 0:
            if not (row.get("early_hrs")):
                if row.get("status") == "Absent":
                    ...
                    row["total_pay_hrs"] = row.get("net_wrk_hrs") + (
                        row.get("ot_hours") or timedelta(0)
                    )
        ...
        if row.shift_hours:
            ...
            if row.status not in ["Leave Without Pay", "Absent"]:
                if (
                    row.net_wrk_hrs.total_seconds() > shift_hours_in_sec
                    or (shift_hours_in_sec - row.net_wrk_hrs.total_seconds()) < 60
                ):
                    row.net_wrk_hrs = timedelta(hours=row.shift_hours)
            leave_status = frappe.db.get_value(
                "Leave Type", {"name": row.status, "is_earned_leave": 1}, ["name"]
            )
            if leave_status:
            ...
            shift_det = frappe.db.get_value(
                "Shift Type",
                shift,
                ["shift_hours", "start_time", "end_time"],
                as_dict=1,
            )
            shift_hours = flt(shift_det.get("shift_hours") or 0)
            shift_name = f"{format_time(shift_det.get('start_time'))} To {format_time(shift_det.get('end_time'))}"
            ...
            leave_status = frappe.db.get_value(
                "Leave Type",
                {"name": row.get("status"), "is_earned_leave": 1},
                ["name"],
            )
            e_leave_status = frappe.db.get_value(
                "Leave Type",
                {"name": row.status, "max_continuous_days_allowed": [">", 0]},
                ["name"],
            )

            # FIXED: Check if LWP specifically
            is_lwp = frappe.db.get_value(
                "Leave Type", {"name": row.status, "is_lwp": 1}, ["name"]
            )
    ...
    for date in date_range:
        ...
        temp = {"login_date": date, "shift": shift_name, "status": status}
...
def get_totals(data, employee):
    ...
    for key, value in totals.items():
        if key in ["status", "shift_hours"]:
            continue
        total_days[key] = (
            flt(value.total_seconds() / conversion_factor, 2)
    ...
    refund = {
        ...
        "total_pay_hrs": min(
            frappe.db.get_value("Employee", employee, "allowed_personal_hours")
            or timedelta(0),
    ...
    net_pay_hrs = {
        "ot_hours": "Net Hrs",
        "total_pay_hrs": totals["net_wrk_hrs"]
        + totals["ot_hours"]
        + refund["total_pay_hrs"]
        - penalty_hrs,
    }
    ...
    net_pay_days = {
        "ot_hours": "Net Days",
        "total_pay_hrs": flt(
            net_pay_hrs["total_pay_hrs"].total_seconds() / conversion_factor, 2
        )
        if conversion_factor
        else 0.0,
    }
    ...
    net_pay_days_wo_ot = {
        ...
        "total_pay_hrs": flt(
            net_pay_hrs_wo_ot["total_pay_hrs"].total_seconds() / conversion_factor, 2
        )
        if conversion_factor
        else 0.0,
    }
