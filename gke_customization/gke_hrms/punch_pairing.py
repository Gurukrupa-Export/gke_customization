...
Rollout switch (HR Settings):
  enable_session_pairing  master switch - ON: engine classifies punches;
...
def get_mode() -> str:
    enabled = frappe.db.get_value("HR Settings", None, "enable_session_pairing")
    return "live" if cint(enabled) else "off"
