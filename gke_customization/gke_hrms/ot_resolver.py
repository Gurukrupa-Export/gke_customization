...
def apply_error_context(mil_doc) -> None:
    ...
    try:
        ...
        def _put(field, value):
            # persist directly for any EXISTING card (draft or submitted);
            # plain attribute set only while the card is first being created
            if mil_doc.name and frappe.db.exists(mil_doc.doctype, mil_doc.name):
                mil_doc.db_set(field, value, notify=False)
            else:
                setattr(mil_doc, field, value)

        _put("punch_error", att.punch_error or "")
        _put("error_case", classify_error_case(att) or "")
        _put("punch_ledger", build_punch_ledger(att.name))

        ot = get_approved_ot(mil_doc.employee, mil_doc.attendance_date)
        if att.punch_error:
            _put("ot_check_status", OT_MATCHED if ot else OT_NONE)
            if not mil_doc.resolution_status:
                _put("resolution_status", RES_PENDING)
        elif getattr(mil_doc, "resolution_status", "") == RES_PENDING:
            # the day self-healed (late punch arrived / attendance regenerated)
            # -> clear the stale pending state
            _put("resolution_status", "")
            ...
            if (
                ...
                _put("ot_check_status", OT_EXCEEDS)
            else:
                _put("ot_check_status", OT_MATCHED)
        else:
            _put("ot_check_status", "")
    except Exception:
        frappe.log_error(
...
def ensure_mil(employee, attendance_date) -> str | None:
    """Get-or-create the Monthly In-Out Log card for an employee-date.
    Creation auto-populates it (validate -> populate_from_attendance ->
    apply_error_context), so error days appear in the report by themselves."""
...
def _update_mil(employee, attendance_date, updates: dict):
    """Push resolution fields to the MIL card — creating it first if needed
    (auto-resolutions must always land on a visible card) — then re-populate
    so the ledger / OT check reflect the resolved state without a Fetch."""
